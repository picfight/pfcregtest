...
func genSpend(t *testing.T, r *coinharness.Harness, amt pfcutil.Amount) *chainhash.Hash {
...
func TestBallance(t *testing.T) {
	...
	expectedBalance := pfcutil.Amount(7200 * pfcutil.AtomsPerCoin)
	actualBalance := coinharness.GetBalance(t, r.Wallet).TotalSpendable.(pfcutil.Amount)
...
func TestSendOutputs(t *testing.T) {
	...
	// input.
	txid := genSpend(t, r, pfcutil.Amount(5*pfcutil.AtomsPerCoin))
	...
	txid = genSpend(t, r, pfcutil.Amount(1000*pfcutil.AtomsPerCoin))
	blockHashes, err = r.NodeRPCClient().Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
